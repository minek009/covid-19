...
class APIwrapper:
    ...
    def get_page(self, filters={}, page_size=5):
        ...
        curr_time = time.time() # added whilst debugging as running this in the notebook, curr_time was not defined (there was no issues using the wrapper seperately to save the Json file in another notebook)
        print(f"Current time: {curr_time}")  # Debugging line to check time
        if page_size>365:
        ...
        print(response) #addedd to to issues fetching data
        if 'results'in response:
            for entry in response['results']:
                if 'value' not in entry:
                    print(f"Missing 'value' in entry: {entry}") # added for debugging due to issues fetching data
